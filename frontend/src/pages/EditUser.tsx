...
const formSchema = z
  .object({
    ...
    senha: z
      ...
      .optional()
      .or(z.literal("")),
    confirmPassword: z.string().max(6, "A senha deve ter no máximo 6 caracteres").optional().or(z.literal("")),
  })
...
export function EditUser() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        if (id) {
          const userData = await getUser(Number.parseInt(id))
          form.reset({
            nome: userData.nome,
        ...
        setToast({
          open: true,
          variant: "error",
          message: "Erro ao carregar dados do usuário. Por favor, tente novamente.",
        })
      }
    }
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      if (id) {
        ...
        await updateUser(Number.parseInt(id), updateData)
        setToast({
          open: true,
          variant: "success",
      ...
      if (axios.isAxiosError(error)) {
        ...
        setToast({
          open: true,
          variant: "error",
          message: errorMessages.length > 0 ? errorMessages.join(". ") : "Erro ao atualizar usuário.",
        })
      } else {
        setToast({
          open: true,
